...
export default function TrendingStories() {
  ...
  return (
    <section className="px-4 md:px-12 py-8">
      <h2 className="font-heading text-2xl md:text-3xl font-bold text-text-primary mb-6">Books</h2>
        ...
        {books.map((book) => (
          <Link key={book._id || book.id} to={`/book/${book._id || book.id}`} className="group">
            <img
              ...
              alt={book.title}
              className="w-full aspect-[3/4] object-cover rounded-xl shadow-md group-hover:shadow-xl group-hover:scale-105 transition-all duration-300"
              loading="lazy"
            />
          </Link>
        ))}
